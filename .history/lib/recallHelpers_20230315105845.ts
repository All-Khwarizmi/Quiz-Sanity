...
export const memoDate: MemoDate = () => {
  let memoObj: MemoDateData = Object();
  memoObj.calendar = calendar();
  memoObj.lastRecallDay = Date();
...
export const memoParser: MemoParser = (memoDateStr) => {
  let memo: MemoDate = JSON.parse(memoDateStr);
...
type MemoDateChecker = (memoName: string) => IsMemoTimeReturn;
const memoDateChecker: MemoDateChecker = (memoName) => {
  ...
  if (isMDInLocalStorage) {
    memoDateCheckerReturn.memoDate = memoParser(isMDInLocalStorage);
  }
  const isTimeObj = isMemoTime();
};
