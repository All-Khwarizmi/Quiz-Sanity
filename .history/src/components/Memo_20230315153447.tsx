            ...
            return (
              <Link key={memo._id} href={`/memo/${params.classe}/${memo.name}`}>
                {!isTime.isMemoDate || isTime.memoDate?.isMemotime ? (
                  <div className='flex flex-row text-sm  space-x-3 justify-between'>
                    <p
                      className={`uppercase italic text-gray-600 animate-pulse`}
                    >
                      <span className='font-bold text-cyan-500'> +50</span> puntos
                    </p>
                  </div>
                ) : (
