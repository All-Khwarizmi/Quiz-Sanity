      ...
      <section className='h-full w-full flex flex-col space-y-5 justify-center items-center'>
        <h1 className='pt-10 font-bold text-5xl uppercase'>
         ...
         {quizRef.current.questions.map((question, index) => {
            ...
            return (
              <div key={Math.floor(Math.random() * 1000)} className={clsx(``)} >
                {/*  Questions */}
                ...
                <div className='grid gap-5 grid-cols-2 opacity-100'>
                  <div
                    className={clsx(
                      `font-bold text-center w-full ${count === index ? null : "hidden"} text-xl hover:border-sky-500  hover:scale-125 hover:text-sky-500 uppercase border-2 rounded-lg py-5 px-3`
                    )}
                  >
                    {question.answerA}
                  </div>
