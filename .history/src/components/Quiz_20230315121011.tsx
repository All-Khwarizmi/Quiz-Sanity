...
import { memoDateChecker, setMemoDate } from '../../lib/recallHelpers';
...
const Quiz = (quiz: QuizType) => {
  ...
  if (memoDate.current.isMemoDate && is) {
    let memoDateInLocalStorage = useRef(JSON.parse(localStorage.getItem(quiz.memo)!));
    memoDateInLocalStorage.current.
  }
