...
const Quiz = (quiz: QuizType) => {
  ...
  console.log('QuizRef', quizRef.current.questions.map(item => console.log(item._id)));
  return (
    <>
      <section className='h-full w-full flex flex-col space-y-5 justify-center items-center'>
        <h1 className='pt-10 font-bold text-5xl uppercase'>
         ...
         {quizRef.current.questions.map(question => {
            return (
              <div key={question._key} className='min-h-96 w-96 py-5 bg-gray-900 flex flex-col rounded-lg items-center justify-center space-y-10 opacity-80'>
                {/*  Questions */}
